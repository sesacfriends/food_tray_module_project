    ...
    while True:
        ...
        try:
            if face is not None and len(face) > 0: # 얼굴 감지 확인
                ...
                if confidence > 80:
                    cv2.putText(image, "Enjoy your meal", (200, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
                    cv2.imshow('FACE RECOGNITION', image)
                else:
                    cv2.putText(image, "You are not our member", (200, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                    cv2.imshow('FACE RECOGNITION', image)

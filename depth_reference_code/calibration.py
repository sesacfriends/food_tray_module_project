...
def run_on_chip_calibration(speed, scan):
    ...
    try:
        ...
        table, health = adev.run_on_chip_calibration(args, progress_callback, 60000)
...
def calculate_target_z(target_size):
    number_of_images = 50  # 캡처할 프레임 수 (최소 10개 이상)
    timeout_s = 30  # 캡처 시간 제한 (초 단위)
    default_z = 1000  # 기본 거리 값 (1m)
    ...
    try:
        ...
        # 타임아웃 또는 충분한 프레임 수 캡처 시 종료
        while counter < number_of_images:
            time.sleep(0.5)
            elapsed = time.time() - stime
            if elapsed > timeout_s:
                raise RuntimeError(f"Timeout: Unable to capture {number_of_images} frames within {timeout_s} seconds. Captured: {counter}")

        # Auto-calibrated device 초기화
        adev = dev.as_auto_calibrated_device()
        try:
            # 타겟 거리 계산
            target_z = adev.calculate_target_z(q, q2, q3, target_size[0], target_size[1])
            print(f'Calculated distance to target: {target_z} mm')
            return target_z
        except RuntimeError as e:
            print(f"Error while calculating target_z: {e}")
            print(f"Using default fallback distance of {default_z} mm.")
            return default_z  # 기본 거리 값 반환
    finally:
        pipe.stop()

            ...
            while True:
                ...
                for obj_id, obj_data in current_frame_objects.items():
                    ...
                    if self.candidate_objects[obj_id] >= 15 and obj_id not in self.confirmed_objects:
                        ...
                        # # object_weight = 0  # 기본값 설정
                        # try:
                        #     # object_weight = gw.object_weight  # 무게 가져오기
                        #     # print(f"아직 무게 없니 ㅠ {object_weight}")
                        #     for cid, cdata in self.confirmed_objects.items():
                        #         if cdata["obj_name"] == obj_name:
                        #             cdata["weight"] = object_weight
                        #             print(f"Updated weight for name: {obj_name} (id: {cid}), weight: {object_weight}g")
                        #             break
                        #     else:
                        #         print(f"Object with name '{obj_name}' not found in confirmed_objects.")
                        # except AttributeError:
                        #     print("Failed to update weight for the current object")
                        # # region, volume, bbox, weight = obj_data  # 데이터 unpacking

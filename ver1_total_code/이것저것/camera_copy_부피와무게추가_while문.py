                ...
                for obj_id, obj_data in current_frame_objects.items():
                    ...
                    if self.candidate_objects[obj_id] >= 15 and obj_id not in self.confirmed_objects:
                        ...
                        # time.sleep(3)
                            ...
                            # 안정된 무게 값을 확인하기 위한 루프
                        start_time = time.time()
                        # stable_weight = None
                        while time.time() - start_time < 3:  # 3초 동안 무게 안정화
                            onair_recent_weight = round(float(gw.recent_weight_str), 2)
                            # if stable_weight is None or abs(onair_recent_weight - stable_weight) <= 0.05:  # 안정된 상태 기준
                                # stable_weight = onair_recent_weight
                            time.sleep(0.1)  # 너무 자주 확인하지 않도록 짧은 대기
                        print(f'first_weight : {gw.first_object_weight}')
                        print(f"안정된 무게값: {onair_recent_weight}")
